...
public class MemberAuth extends HttpServlet {
	...
	//서블릿 객체 생성시 최초1번만 호출되는 수명주기 메서드
	@Override
	public void init() throws ServletException {
		/*
		 application내장객체를 얻어온다. 모델2 방식에서는 서블릿이 먼저 요청을
	...
	/*
	 service()메서드는 get방식, post방식 둘다 요청을 받을 수 있으므로 해당 메서드
	 내에서 요청을 처리할 수 있다.  
	 */
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		//회원 이름을 통해 로그인 성공 여부를 판단
		String memberName= memberDTO.getName();
		if(memberName!=null) {
			//로그인에 성공한 경우 
			req.setAttribute("authMessage", memberName+" 회원님 해위");
		}
		else {
			//로그인에 실패한 경우
