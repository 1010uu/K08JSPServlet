...
<head>
    ...
    <script type="text/javascript">
        function idUse(user_id) {
            /*
            부모창에서 팝업창을 열 때 readonly 속성이 적용돼 사용자는
            내용을 수정할 수 없지만, JS를 이용하면 수정할 수 있다.
            또한 opener 속성을 통해 부모창으로 필요한 데이터를 전달할 수 있다.
            */
            opener.document.registFrm.id.value = user_id;
            self.close();
        }
    </script>
</head>
<body>
    <h2>아이디 중복 확인하기</h2>   
    <h3>부모 창에서 입력한 아이디: <%=request.getParameter("id") %></span></h3>
<%
if(request.getParameter("id").equals("kosmo")){
%>
    <h3>아이디가 중복되므로 재입력해주세용</h3> 
