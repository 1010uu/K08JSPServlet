...
쿠키
	...
	PC에 파일 형태로 저장된다.
	쿠키는 300개까지 만들수 있고, 하나의 크기는 최대 4kb이다. 
 -->
	<h2>1.쿠키(Cookie) 설정</h2>
	<%
	/*
	쿠키는 생성자를 통해서만 생성할 수 있다. setName()이란 메서드는
	...
	*/
	...
	//쿠키 유지 시간을 설정. 60*60= 1시간으로 설정하였다.
	cookie.setMaxAge(3600);
	//응답헤더에 쿠키를 추가하여 클라이언트측으로 전송한다.
	response.addCookie(cookie);
	//여기까지의 코드를 통해 클라이언트 측에는 쿠키가 생성된다.
	%>
	
	<h2>2.쿠키 설정 직후 쿠기값 확인하기</h2>
	<%
	/*
	request 내장 객체의 getCookie()메서드를 통해 현재 생성된 모든 쿠키를
	배열의 형태로 가져온다.
	*/
	...
	if(cookies!=null){
		...
		for(Cookie c : cookies){
			String cookieName = c.getName(); //쿠키명 읽어오기
