...
<body>
	...
	<%
		request.removeAttribute("requestString"); //정상적으로 삭제됨.
		request.removeAttribute("requestInteger");//존재하지 않는 정보로 아무런 일도 발생되지 않음.
	%>
	...
	%>
	<ul>
		<li>String 객체 : <%= request.getAttribute("requestString") %></li><!-- null 출력 -->
		<li>Person 객체 : <%= rPerson.getName()%>, <%= rPerson.getAge() %></li><!-- 정상 출력 -->
	</ul>
	...
	포워드 : 현재 페이지로 들어온 요청을 다음 페이지로 보내는 기능을 말한다.
		...
		포워드 된 페이지의 내용만 화면에 출력되는 특징이 있다.
	*/
